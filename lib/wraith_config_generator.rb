...
class WraithConfigGenerator
  def initialize
    ...
    pages_filename = "#{ROOT_DIR}/config/govuk_pages.yml"
    raise "Cannot find file config/govuk_pages.yml: run 'rake update_page_list' to generate." unless File.exist?(pages_filename)
    @govuk_pages = YAML.load_file(pages_filename)
  ...
  def save
    File.open("#{ROOT_DIR}/config/wraith.yaml", 'w') do |fp|
